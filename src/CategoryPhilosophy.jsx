...
export default function CategoryPhilosophy({ books, selectedLanguage, scaleUp, scaleDown, currentTranslations, translateCategory, truncate, handleBookSelect }) {
    ...
    return (<>
        <div>
            <h2>
                <a>{currentTranslations.philosophy}{currentTranslations.books} <span>&rsaquo;</span></a>
            </h2>
        </div>
        <div className='slider-container'>
            <div className='slider-wrapper'>
                <div className='books-slider' style={{ marginLeft: `${sliderIndex * sliderShiftValue + initialBooksSliderMargin}px` }}>
                    {books.filter((book) => book.genre === 'Philosophy').slice(0, 10).map((book, index) => (
                        <div key={'Philosophy' + index} className='books-item'
