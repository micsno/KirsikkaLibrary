...
export default function CategoryMystery({ books, selectedLanguage, scaleUp, scaleDown, currentTranslations, translateCategory, truncate, handleBookSelect }) {
    ...
    return (<>
        <div>
            <h2>
                <a>{currentTranslations.mystery}{currentTranslations.books} <span>&rsaquo;</span></a>
            </h2>
        </div>
        <div className='slider-container'>
            <div className='slider-wrapper'>
                <div className='books-slider' style={{ marginLeft: `${sliderIndex * sliderShiftValue + initialBooksSliderMargin}px` }}>
                    {books.filter((book) => book.genre === 'Mystery').slice(0, 10).map((book, index) => (
                        <div key={'Mystery' + index} className='books-item'
