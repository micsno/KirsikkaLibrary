...
export default function CategoryHorror({ books, selectedLanguage, scaleUp, scaleDown, currentTranslations, translateCategory, truncate, handleBookSelect }) {
    ...
    return (<>
        <div>
            <h2>
                <a>{currentTranslations.horror}{currentTranslations.books} <span>&rsaquo;</span></a>
            </h2>
        </div>
        <div className='slider-container'>
            <div className='slider-wrapper'>
                <div className='books-slider' style={{ marginLeft: `${sliderIndex * sliderShiftValue + initialBooksSliderMargin}px` }}>
                    {books.filter((book) => book.genre === 'Horror').slice(0, 10).map((book, index) => (
                        <div key={'Horror' + index} className='books-item'
