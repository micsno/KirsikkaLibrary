...
export default function CategoryChildrens({ books, selectedLanguage, scaleUp, scaleDown, currentTranslations, translateCategory, truncate, handleBookSelect }) {
    ...
    return (<>
        <div>
            <h2>
                <a>{currentTranslations.childrens}{currentTranslations.books} <span>&rsaquo;</span></a>
            </h2>
        </div>
        <div className='slider-container'>
            <div className='slider-wrapper'>
                <div className='books-slider' style={{ marginLeft: `${sliderIndex * sliderShiftValue + initialBooksSliderMargin}px` }}>
                    {books.filter((book) => book.genre === 'Children').slice(0, 10).map((book, index) => (
                        <div key={'Childrens' + index} className='books-item'
